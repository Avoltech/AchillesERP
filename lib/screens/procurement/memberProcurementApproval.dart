...
class _MemberProcurementToApproveState extends State<MemberProcurementToApprove> {
  ...
  Future getForms() async {
    ...
    //FORM     A
    QuerySnapshot qs1 = await Firestore.instance.collection('forms').where('formType', isEqualTo: 'A').where('status', isEqualTo: 'stage3').where('marketSurveyApprovalBy', isEqualTo: widget.user.uid).getDocuments();
