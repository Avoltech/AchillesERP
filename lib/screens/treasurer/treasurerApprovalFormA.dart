...
class TreasurerApprovalFormA extends StatefulWidget {
  final DocumentSnapshot post;
  final User user;
  TreasurerApprovalFormA({Key key, this.post, this.user}) : super(key: key);

  @override
  _TreasurerApprovalFormAState createState() => _TreasurerApprovalFormAState();
}

class _TreasurerApprovalFormAState extends State<TreasurerApprovalFormA> {
  ...
  Widget build(BuildContext context) {
    ...
    showDisapprovalAlertDialog(BuildContext context, List _tempRemarks) async{
      ...
      Widget member = FlatButton(
        ...
        onPressed: () {
          setState(() async {
            loading = true;
            await db.collection('forms').document(widget.post.documentID).updateData({'approval1': 0, 'approval2': 0, 'marketSurveyApproval': 0, 'marketSurveyApprovalBy': '', 'marketSurveyApprovalUid': '', 'marketSurveyReceiver': '', 'status': 'stage-1', 'returned': true, 'remarks': _tempRemarks});
      ...
      Widget head = FlatButton(
        ...
        onPressed: () {
          setState(() async {
            ...
            await db.collection('forms').document(widget.post.documentID).updateData({'approval1': 0, 'approval2': 0, 'marketSurveyApproval': 0, 'marketSurveyApprovalBy': '', 'marketSurveyApprovalUid': '', 'marketSurveyReceiver': '', 'status': 'stage0', 'returned': true, 'remarks': _tempRemarks, 's0c': currentTime});
            showAlertDialog(context, 'disapproved');
          });
        },
      );
      Widget procurement = FlatButton(
        ...
        onPressed: () {
          setState(() async {
            loading = true;
            await db.collection('forms').document(widget.post.documentID).updateData({'marketSurveyApproval': 0, 'marketSurveyApprovalBy': '', 'marketSurveyApprovalUid': '', 'marketSurveyReceiver': '', 'status': 'stage1', 'returned': true, 'remarks': _tempRemarks, 's1c': currentTime});
            showAlertDialog(context, 'disapproved');
          });
        },
      );
    ...
    approve() async{
      ...
      var currentTime = DateTime.now();
      await db.collection('forms').document(widget.post.documentID).updateData({'status': 'stage3','treasurerApproval' : 1, 'treasurerApprovalBy': widget.user.name, 'treasurerApprovalByUid': widget.user.uid, 's3c': currentTime});
  ...
  marketSurveyList() {
    ...
    if(widget.post.data['marketSurvey'].length > 0) {
      ...
      return Text('no market survey done');
    }
  }
}
