...
import 'package:achilleserp/screens/qc/qcApprovalFormA.dart';

class QcToApproveList extends StatefulWidget {
  final User user;
  QcToApproveList({Key key, this.user}) : super(key: key);

  @override
  _QcToApproveListState createState() => _QcToApproveListState();
}

class _QcToApproveListState extends State<QcToApproveList> {

  Future _data;

  Future getForms(String uid) async {
    QuerySnapshot qs1 = await Firestore.instance.collection('forms').where('qcApproval', isEqualTo: 0).where('status', isEqualTo: 'stage4').getDocuments();
  ...
  Future getTimeCounters() async{
    ...
    stageTimeHours = counterSnapshot.data['stage4'];
                    ...
                    if(snapshot.data.length == 0) {
                      ...
                      return ListView.builder(
                        ...
                        itemBuilder: (_, index) {
                          ...
                          var s4c = snapshot.data[index].data['s4c'];
                          print(currentTime);
                          print(s4c.toDate());
                          print(currentTime.difference(s4c.toDate()).inMinutes);
                          var diffMins = currentTime.difference(s4c.toDate()).inMinutes;
                          print(diffMins);
                              ...
                              onTap: () {
                                if ((stageTimeHours * 60) - diffMins < 0) {
                                  ...
                                  if (snapshot.data[index].data['formType'] == 'A') {
                                    Navigator.push(
                                        context,
                                        MaterialPageRoute(builder: (context) => QcApprovalFormA(post: snapshot.data[index], user: widget.user))
                                    );
                                  }
                                }
